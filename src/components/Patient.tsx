...
const DoctorView = () => {
  ...
  return (
    <>
      <h2 className="mb-8 self-center text-2xl font-bold">Welcome Patient, {userData?.user.name}</h2>
      ...
      <Card className="m-4 flex min-h-64 w-64 flex-col self-center rounded-lg bg-white p-5 text-center shadow-lg">
        {isLoading ? <p>loading...</p> : data?.[0] ? <UpcomingSession data={data[0]} /> : <p>No Sessions</p>}
      </Card>
      <div className="mx-8 my-4 flex flex-col self-center">
        <Link href={"/view"}>
